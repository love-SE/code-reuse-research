...
if os.path.isfile(r"F:\\CCFinder\\outName.txt"):
    os.remove(r"F:\\CCFinder\\outName.txt")

allCommit = [0 for i in range(100000)]
hashName = {}
res = [0 for i in range(100000)]
at = 0
...
#projects = ['\\spring-cloud-alibaba','\\springfox','\\VirtualXposed','\\android-advancedrecyclerview','\\android-betterpickers','\\android-oss']
for project in projects:
    ...
    def readCommit(fileInfo):
        ...
        path = path.replace('D:/NewData','F:/CCFinder/message')
        path = path + '/0.txt'
        return time,fileName[fileIndex],path
        return "2008-01-01"
        ...
        if line == 'source_files {\n':
            while 1:
                ...
                if "NewData" in listName[1]:
                    ...
                    linem = linem.strip('\n')
                    name = linem[5:]
                    if hashName.__contains__(name):
                        t = hashName[name]
                        allCommit[t] = allCommit[t] + 1
                    else:
                        hashName[name] = at
                        allCommit[at] = 1
                        at = at + 1
    ...
    for i in range(len(clonePairs)//2):
        ...
        time2,Commitfile,path = readCommit(listPair[2])
...
Output = open(r"F:\\CCFinder\\outName.txt",'a+',encoding='UTF-8')
for i in hashName:

    Output.write(str(hashName[i]))
    Output.write('\t')
    Output.write(i)
    Output.write('\t')
    Output.write(str(allCommit[hashName[i]]))
    Output.write('\t\t')
    Output.write(str(res[hashName[i]]))
    Output.write('\n')
Output.close()
